...
public class HttpClient {
    ...
    public void checkConnection(RequestConfiguration configuration, String token) {
        String url = configuration.getPartnerConfiguration().getProperty(Constants.PartnerConfigurationKeys.URL);
        Header[] headers = createHeaders(token);

        StringResponse response = get(url, headers);
    ...
    public Transaction createTransaction(RequestConfiguration configuration, MoneyTrackPaymentRequest request) {
        String url = configuration.getPartnerConfiguration().getProperty(Constants.PartnerConfigurationKeys.URL);
        Header[] headers = createHeaders(configuration.getContractConfiguration().getProperty(Constants.ContractConfigurationKeys.MERCHANT_API_TOKEN).getValue());
    ...
    public Transaction getTransaction(RequestConfiguration configuration, String partnerTransactionId) {
        String url = configuration.getPartnerConfiguration().getProperty(Constants.PartnerConfigurationKeys.URL)
                + "/"
    ...
    private Transaction handleResponse(StringResponse response) {
        if (response.isSuccess()) {
            // create a transaction and return it
            return parser.fromJson(response.getContent(), Transaction.class);
        } else if (response.getStatusCode() == 401) {
            // throw an InvalidDataException because of a bad token
            throw new InvalidDataException("Invalid token");
        } else {
            // throw an InvalidDataException with the first error given in response body
            String errorMessage = PluginUtils.getErrorMessage(response.getContent());
